...
export function VideoPlayer({
  ...
  const handleLoadedMetadata = () => {
    if (videoRef.current) {
      ...
      if (saved) {
        const time = Number.parseFloat(saved)
        if (!isNaN(time) && time > 5 && time < videoRef.current.duration - 10) {
          setSavedTime(time)
      ...
      {showResumePrompt && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/70 text-white">
          <div className="bg-gray-800 p-6 rounded-lg max-w-md text-center">
            <h3 className="text-lg font-medium mb-2">继续上次观看？</h3>
            <p className="text-sm opacity-80 mb-4">您上次观看到 {formatTime(savedTime)}</p>
